...
var _require = require('./logic'),
    ...
    ytSearchUploader = _require.ytSearchUploader,
    whiteListEngine = _require.whiteListEngine;
...
program.command('youtube').alias('yt').description('Search for Youtube Videos').action(async function (args) {
  try {
    var answers = await prompt(questions.ytQuestions);
...
program.command('youtubeUploader').alias('ytU').description('Search for Youtube Videos and Upload to CMS').action(async function (args) {
  try {
    var answers = await prompt(questions.ytQuestions);
...
program.command('whiteListEngine').alias('white').description('Create a white list').action(async function () {
  try {
    await checkForAuth();
    var answers = await prompt(questions.nListQuestions);
